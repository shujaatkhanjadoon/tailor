      ...
      if (editingId) {
        ...
        await (supabase as any)
          ...
          .update({
            ...
            pin_hash:      pinToSave,
            ...(pinChanged ? { pin_plain: await fetch('/api/team/encrypt-pin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ pin: form.pin }) }).then(r => r.json()).then(d => d.encrypted) } : {}),
        ...
        const { error: sbError } = await (supabase as any)
          ...
          .insert({
            ...
            role:           'karigar',
            pin_hash:       pinToSave,
            pin_plain:      await fetch('/api/team/encrypt-pin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ pin: form.pin }) }).then(r => r.json()).then(d => d.encrypted),
            speciality:     memberData.speciality,
            pay_rate_type:  memberData.payRateType ?? null,
            pay_rate:       memberData.payRate,
            is_active:      true,
