...
export default function NewOrderPage() {
  ...
  const handleFinalSubmit = async () => {
    ...
    if (!data.customerId || !data.garmentType || !data.dueDate || !data.totalPrice) {
    ...
    try {
      ...
      const order = await orderOps.add(shopId, {
        ...
        trackingCode:        data.customerName!,
      ...
      const filled = Object.fromEntries(
        Object.entries(data.measurements ?? {}).filter(([, v]) => v && v !== '0')
      )
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center px-6 text-center">
        ...
        <div className="bg-slate-100 rounded-2xl px-4 py-2 mb-6 max-w-xs w-full">
          <p className="text-xs text-slate-500 font-mono truncate">{trackUrl}</p>
        </div>
