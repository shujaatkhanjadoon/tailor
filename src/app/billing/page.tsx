    ...
    <div className="min-h-screen bg-slate-50 pb-20 lg:pb-8">
      ...
      <div className="px-4 pt-5 space-y-4">
        ...
        <div className={cn(
          ...
          plan.plan === 'business'  ? 'bg-purple-600 border-purple-600' :
          plan.plan === 'professional' ? 'bg-blue-600 border-blue-600' :
                                         'bg-slate-100 border-slate-200'
        )}>
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className={cn('text-sm font-medium', plan.plan === 'starter' ? 'text-slate-500' : 'text-white/70')}>
                Current Plan
              </p>
              <h2 className={cn('text-2xl font-bold mt-0.5', plan.plan === 'starter' ? 'text-slate-800' : 'text-white')}>
                {planDef.emoji} {planDef.name}
              </h2>
            </div>
            <div className={cn(
              'px-3 py-1.5 rounded-full text-xs font-bold',
              plan.isTrial  ? 'bg-white/20 text-white' :
              plan.isActive ? 'bg-white/20 text-white' :
                              'bg-red-100 text-red-700'
            )}>
              {plan.isTrial   ? '✨ Trial'    :
               plan.inGrace   ? '⚠️ Grace'    :
            ...
            return (
              <div key={m.label}>
                <div className="flex justify-between text-xs mb-1.5">
                  ...
                  <span className={cn('font-bold', isNearLimit ? 'text-amber-600' : 'text-slate-700')}>
                    {m.used}{m.limit ? ` / ${m.limit}` : ' (unlimited)'}
                  </span>
                </div>
                {m.limit && (
                  <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      className={cn('h-full rounded-full', isNearLimit ? 'bg-amber-500' : m.color)}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                )}
