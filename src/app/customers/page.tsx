      ...
      <main className="flex-1 px-4 pt-4 pb-4">
        ...
        {!isLoading && filtered.length > 0 && (
          <div className="space-y-3">
            ...
            {filtered.map(customer => <CustomerCard key={customer.id} customer={customer} onClick={() => router.push(`/customers/${customer.id}`)} />)}
            {filtered.length >= 10 && (
              <p className="text-center text-xs text-slate-400 py-3">{t('customers.countShowing', { count: filtered.length })}</p>
            )}
            {hasMore && (
              <button onClick={loadMore}
                className="w-full rounded-2xl border border-slate-200 bg-white py-3 text-sm font-bold text-slate-600 transition-colors hover:bg-slate-50">{t('customers.loadMore')}</button>
            )}
          </div>
        )}
